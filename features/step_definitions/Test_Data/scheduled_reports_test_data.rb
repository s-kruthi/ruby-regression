# ######### Scheduled Reports page data #############################

SCHD_REPORTS_NAME_VALUE = "This is a test. Please DO NOT modify"

SCHD_REPORTS_FREQ_VALUE = "Daily"

SCHD_REPORTS_START_DATE_VALUE = "01/10/2016"
SCHD_REPORTS_END_DATE_VALUE = "01/10/2018"

REPORTS_NAME_FILTER_INDEX_VALUE = 17 #Learner Report

SCHD_REPORTS_FILTER_INDEX_VALUE = 0 #Part time Filter

REPORTS_LEVEL_VALUE = "Course"
REPORTS_FORMAT_VALUE = "CSV"

RECIPIENT1_FIELD_VALUE = "test1" #First Recipient
RECIPIENT2_FIELD_VALUE = "test2" #Second Recipient

SHOW_ALL_DATA_VALUE = "Enabled"


# CLIENT_CONTRACT_VALUE = "staging" + rand(20..100).to_s
# CONTRACT_START_DATE = "01/01/2017"
# ENTER_CONTRACT_END_DATE = "No"
# CONTRACT_END_DATE = "01/02/2017"
# UNLIMITED_USER_VALUE = "No"
# SUBSCRIBER_NUMBER_VALUE = "200"
# CURRENCY_OPTION_VALUE = "USD"
# EXCESS_FEE_VALUE = "1000"
# USER_TOLERANCE_VALUE = "20"
# PAYMENT_TERMS_VALUE = "This is a Test. Please DO NOT modify"
# STORAGE_ALLOC_VALUE = "100"
# BANDWIDTH_ALLOC_VALUE = "300"
# IS_CC_CLIENT_VALUE = "Yes"
# PRODUCT_OPTION_VALUE_1 = "Recruitment"
# PRODUCT_OPTION_VALUE_2 = "Learning"
# PRODUCT_OPTION_VALUE_3 = "Performance"
#
# CONTACT_FIRSTNAME_VALUE = "Test"
# CONTACT_LASTNAME_VALUE = "Team"
# CONTACT_EMAIL_VALUE = "test1.omar1@elmodev.com"
# CONTACT_ADDRESS_VALUE = "This is a test. Please DO NOT modify"
#
# CONTRACT_SEARCH_VALUE = "Staging4"
#
# CONTRACT_DROPDOWN_ACTION_ITEM = "Edit"
#
# VERIFY_SAVE_SUCCESSFUL_VALUE = "Save Success"