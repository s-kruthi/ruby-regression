...
def ConnectToDatabaseAndValidateTheCourseEnrolmentNotification()
  ...
  begin
    result = %x(mysql -utester -p#{$DB_PWD} #{TMSFULL_DATABASE} -h127.0.0.1 --port 33060 -A < ./features/step_definitions/MySQL_Scripts/sql_commands/learning_course_assignment.sql | tee ./features/step_definitions/MySQL_Scripts/sql_dependencies/myscript.txt) # connect to DB -> run SQL -> save it in text file
      ...
      krs = result.include?  ("user_id: 3472") #true validate for employee got enrolled in the course
      trs = result.include?  ("course_id: 392") #true validate for employee enrolled for the right course
      mrs = result.include?  ("trigger_id: Learning.CourseNewEnrolmentTrigger") #true validate right template got triggered
      qrs = result.include?  ("body: <p>New Enrolment<br />
...
&lt;DYNAMIC&gt;PLACEHOLDER_RETRAIN_PERIOD: 0 month&lt;/DYNAMIC&gt; &lt;-- Retrain period should not be displayed here</p>")
