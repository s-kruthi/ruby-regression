Given(/^I Log In As A (.*)$/i) do |pr_login_name|
  ...
  when "Prod Payroll Admin"
    begin
      Sleep_Until(ProvideLoginDetails(PAYROLL_USER, PAYROLL_PROD_ADMIN_USER))
      Sleep_Until(ProvideLoginDetails(PAYROLL_PSWD, PAYROLL_PROD_ADMIN_PWD))
  ...
  when "Staging Payroll Admin"
    begin
      ...
      sleep(5)
      Sleep_Until(ProvideLoginDetails(PAYROLL_USER, PAYROLL_STAG_ADMIN_USER))
      Sleep_Until(ProvideLoginDetails(PAYROLL_PSWD, PAYROLL_STAG_ADMIN_PWD))
      Sleep_Until(LoginToPayroll(PAYROLL_LOGIN))
    end
