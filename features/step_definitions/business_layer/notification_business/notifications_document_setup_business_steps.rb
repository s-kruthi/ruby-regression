Given(/^I Am On The Document Template List View Page$/) do
  startWebDriver
...
Then(/^I It Should Instantly Trigger An Email Notification$/) do
  ConnectToDatabaseAndValidateTheDocumentAssignedNotifications()
end

Given(/^I Have Assigned Many Documents To A User$/) do
  startWebDriver
...
When(/^I Successfully Submit My Document Template$/) do
...
Then(/^I Should Be Instantly Notified About My Document Awaiting Approval$/) do
  ConnectToDatabaseAndValidateTheDocumentAwaitingNotifications()
end

Given(/^I Am On The My Document Page Of A Manager$/) do
  startWebDriver
  GoToThePage(MY_DOCUMENT_PAGE)
...
When(/^I Review And Reject The Submitted Document$/) do
