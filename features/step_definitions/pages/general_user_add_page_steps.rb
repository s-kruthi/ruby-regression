...
def add_user_details(limit)
  ...
  for loop in i..limit do
    ...
    enter_user_details(NEW_USER_USERNAME_ID, user_name)
    enter_user_details(NEW_USER_EMAIL_ID, email_address)
    select_a_manager(MANAGER_SELECT_DROPDOWN_ID, MANAGER_SELECT_INPUT_ID,MANAGER_SELECT_RESULT_ID) if SELECT_MANAGER.to_i == 1
    sleep (3)
    loop += 1
  end
  puts "Total number of users added: \"#{limit}\"\n"
end
...
def select_a_manager(recipient_field_id, recipient_input_id, recipient1_input_value, recipient1_result_id)
  ...
  sleep(1)
  $driver.find_elements(:id, recipient_input_id).first.send_keys(recipient1_input_value)
  sleep(2)
  $driver.find_elements(:class, recipient1_result_id).first.click
end
