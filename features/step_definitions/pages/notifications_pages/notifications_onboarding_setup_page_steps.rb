...
def ConnectToDatabaseAndValidateTheChangePasswordTriggeredNotifications()
  StartTheTunnel()
  begin
    result = %x(mysql -utester -pMuraf3cAR pmsdev_tmsfull -h127.0.0.1 --port 33060 < ./features/step_definitions/MySQL_Scripts/sql_commands/password_change.sql | tee ./features/step_definitions/MySQL_Scripts/sql_dependencies/myscript.txt) # connect to DB -> run SQL -> save it in text file
...
def ConnectToDatabaseAndValidateTheForgotPasswordTriggeredNotifications()
  StartTheTunnel()
  begin
    result = %x(mysql -utester -pMuraf3cAR pmsdev_tmsfull -h127.0.0.1 --port 33060 < ./features/step_definitions/MySQL_Scripts/sql_commands/password_forgot.sql | tee ./features/step_definitions/MySQL_Scripts/sql_dependencies/myscript.txt) # connect to DB -> run SQL -> save it in text file
    frs = result.include?  ("is_notified: 1") #true validate
